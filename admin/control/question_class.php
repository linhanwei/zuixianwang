...
class question_classControl extends SystemControl{
	...
	public function article_classOp(){
		 ...
		 * 列表
		 */
		$tmp_list = $model_class->getTreeClassList(2,2);
		...
		if ($_GET['ajax'] == '1'){
			...
			Tpl::showpage('question_class.index');
		}
	}
	...
	public function article_class_addOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
                ...
                $insert_array['ac_type'] = 2;

				$result = $model_class->add($insert_array);
				if ($result){
					$url = array(
						array(
							'url'=>'index.php?act=question_class&op=article_class_add&ac_parent_id='.intval($_POST['ac_parent_id']),
							'msg'=>'继续添加',
						),
						array(
							'url'=>'index.php?act=question_class&op=article_class',
							'msg'=>'返回问题列表',
						)
					);
		 ...
		 * 父类列表，只取到第三级
		 */
		$parent_list = $model_class->getTreeClassList(2,2);
		...
		Tpl::showpage('question_class.add');
	}
...
//				$update_array['ac_parent_id'] = intval($_POST['ac_parent_id']);
				...
				if ($result){
					$url = array(
						...
						array(
							'url'=>'index.php?act=question_class&op=article_class_edit&ac_id='.intval($_POST['ac_id']),
							'msg'=>'继续编辑',
						),
					);
					$this->log(l('nc_edit,article_class_index_class').'['.$_POST['ac_name'].']',1);
					showMessage('编辑成功','index.php?act=question_class&op=article_class');
				}else {
					showMessage('编辑失败');
				}
			}
		...
		Tpl::showpage('question_class.edit');
	}
	...
	public function article_class_delOp(){
		...
		if (intval($_GET['ac_id']) > 0){
			...
			showMessage('删除成功','index.php?act=question_class&op=article_class');
		}else {
			showMessage('删除失败','index.php?act=article_class&op=article_class');
		}
	}
	/**
	 * ajax操作
