...
class points_inviterControl extends SystemControl{
	const EXPORT_SIZE = 5000;
	public function __construct(){
		parent::__construct();
		Language::read('points_inviter');
	}
	...
	public function addpointsOp(){
		if (chksubmit()){
			...
			$obj_validate->validateparam = array(
                ...
                array("input"=>$_POST["pl_sn"], "require"=>"true","message"=>'请输入对应单号'),
                array("input"=>$_POST["pointsdesc"], "require"=>"true","message"=>'请输入增加减少积分的原因')
			);
			...
			if ($_POST['operatetype'] == 2 && $pointsnum > intval($member_info['member_points_inviter'])){
				showMessage(Language::get('admin_points_points_short_error').$member_info['member_points_inviter'],'index.php?act=points&op=addpoints','','error');
			}

			$obj_points = Model('points_inviter');
            ...
            try{
                $obj_points->savePointsLog('system',$insert_arr,true);
                $this->log(L('admin_points_mod_tip').$member_info['member_name'].'['.(($_POST['operatetype'] == 2)?'':'+').strval($insert_arr['pl_points']).']',null);
                showMessage(Language::get('nc_common_save_succ'),'index.php?act=points_inviter&op=addpoints');
            }catch (Exception $ex){
                showMessage(Language::get('nc_common_save_fail'),'index.php?act=points_inviter&op=addpoints','','error');
            }
		}else {
			Tpl::showpage('points_inviter.add');
		}
	}
	public function checkmemberOp(){
		...
		if (is_array($member_info) && count($member_info)>0){
			...
			echo json_encode(array('id'=>$member_info['member_id'],'name'=>$member_info['member_name'],'points'=>$member_info['member_points_inviter']));
		}else {
			echo ''; die;
		}
	}
	...
	public function points_inviter_logOp(){
		...
		Tpl::showpage('points_inviter_log');
	}
