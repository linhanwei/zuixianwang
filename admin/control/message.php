...
class messageControl extends SystemControl{
    ...
    public function mobileOp(){
        ...
        if (chksubmit()){
            $update_array = array();
            $update_array['sms_plugin'] 	= $_POST['sms_plugin'];
            $update_array['mobile_host'] 	= $_POST['mobile_host'];
            $update_array['mobile_sid'] 	= $_POST['mobile_sid'];
            $update_array['mobile_token'] 	= $_POST['mobile_token'];
            $update_array['mobile_appid'] 		= $_POST['mobile_appid'];
	...
	public function email_testingOp(){
		...
		$mall_url	= MALL_SITE_URL;

        $site_title = C('site_name');
        $message = '<p>'.$lang['this_is_to']."<a href='".$mall_url."' target='_blank'>".$site_title.'</a>'.$lang['test_email_send_ok'].'</p>';
// 		if ($email_type == '1'){
			$obj_email = new Email();
