...
class orderControl extends SystemControl{
	...
	public function show_orderOp(){
		...
		//订单佣金信息
		$order_info['commis_amount'] = 0;
		if($order_info['order_state'] == ORDER_STATE_SUCCESS){
			$ob_no = date('Ym',$order_info['finnshed_time']) . $order_info['store_id'];
			$commis_list = Model('bill')->getOrderCommissionList(array('ob_no'=>$ob_no),'*',10,'dai_type');
	    	$commis_count = count($commis_list);
	    	$commis_amount = 0;
	    	$member_commis_list = array();		//会员佣金
        	...
        	for($i = 0;$i<$commis_count;$i++){
    			...
    			$tmp['member_id'] = $commis_list[$i]['member_id'];
    			$tmp['member_name'] = $commis_list[$i]['member_name'];
    			$tmp['dai_type'] = $commis_list[$i]['dai_type'];
    			
    			$tmp['commis_amount'] = round($commis_amount - $next_commis,2);
    			$order_info['commis_amount'] += $tmp['commis_amount']; 
    			$member_commis_list[] = $tmp;
        	}
        	
		}
		Tpl::output('member_commis_list',$member_commis_list);
		Tpl::output('order_info',$order_info);
