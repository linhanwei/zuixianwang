...
class ownshopControl extends SystemControl
{
    ...
    public function addOp()
    {
        if (chksubmit())
        {
            ...
            if (!$this->checkSellerName($_POST['seller_name']))
                showMessage('店主卖家账号名称已被其它商户占用', '', 'html', 'error');
            ...
            $this->log("新增自营商户: {$saveArray['store_name']}");
    ...
    public function delOp()
    {
        ...
        if (empty($storeArray)) {
            showMessage('自营商户不存在', '', 'html', 'error');
        }

        if (!$storeArray['is_own_shop']) {
            showMessage('不能在此删除非自营商户', '', 'html', 'error');
        }
        ...
        if ((int) model('goods')->getGoodsCount($condition) > 0)
            showMessage('已经发布商品的自营商户不能被删除', '', 'html', 'error');
        ...
        $this->log("删除自营商户: {$storeArray['store_name']}");
    ...
    public function editOp()
    {
        ...
        if (!$storeArray['is_own_shop']) {
            showMessage('不能在此管理非自营商户', '', 'html', 'error');
        }
        ...
        if (chksubmit()) {
            ...
            */
            ...
            $this->log("编辑自营商户: {$saveArray['store_name']}");
            showMessage('操作成功', urlAdmin('ownshop', 'list'));
        }

        if (empty($storeArray))
            showMessage('商户不存在', '', 'html', 'error');
    ...
    public function bind_class_addOp()
    {
        ...
        if($result) {
            ...
            $this->log('增加自营商户经营类目，类目编号:'.$result.',商户编号:'.$store_id);
    ...
    private function _bind_class_del($bid)
    {
        ...
        */
        ...
        $this->log('删除自营商户经营类目，类目编号:'.$bid.',商户编号:'.$store_bind_class_info['store_id']);
        return $data;
    }
