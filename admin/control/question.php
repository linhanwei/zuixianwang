...
class questionControl extends SystemControl{
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				if ($result){
					...
					$url = array(
						array(
							'url'=>'index.php?act=question&op=article',
							'msg'=>"返回问题列表",
						),
						array(
							'url'=>'index.php?act=question&op=article_add&ac_id='.intval($_POST['ac_id']),
							'msg'=>"继续添加",
						),
					);
			...
			if ($error != ''){
				...
				if ($result){
					...
					$url = array(
						...
						array(
							'url'=>'index.php?act=question&op=article_edit&article_id='.intval($_POST['article_id']),
							'msg'=>'继续编辑',
						),
					);
