...
<div class="index_block home10">
        ...
        <?php if (!empty($item_data['item']) && is_array($item_data['item'])) { ?>
            <?php foreach ($item_data['item'] as $item_key => $item_value) { ?>
                <div nctype="item_image" class="item"><img nctype="image"
                                                           src="<?php echo getMbSpecialImageUrl($item_value['image']); ?>"
                                                           alt="">
                    <?php if ($item_edit_flag) { ?>
                        <input nctype="image_name" name="item_data[item][<?php echo $item_key; ?>][image]" type="hidden"
                               value="<?php echo $item_value['image']; ?>">
                        <input nctype="image_type" name="item_data[item][<?php echo $item_key; ?>][type]" type="hidden"
                               value="<?php echo $item_value['type']; ?>">
                        ...
                        <a nctype="btn_edit_item_image" data-desc="160*260" href="javascript:;"><i
    ...
    <?php if ($item_edit_flag) { ?>
        <a nctype="btn_add_item_image" class="btn-add" data-desc="160*260" href="javascript:;">添加新的模块内容</a>
    <?php } ?>
</div>
