...
<div class="page">
  ...
  <div style="text-align:right;">
      <a class="btns" target="_blank" href="index.php?<?php echo $_SERVER['QUERY_STRING'];?>&op=export_cash_step1"><span><?php echo $lang['nc_export'];?>Excel</span></a>
      <a class="btns" target="_blank" href="index.php?<?php echo $_SERVER['QUERY_STRING'];?>&op=export_cash_step1&bank=ccb"><span><?php echo $lang['nc_export'];?>建行Excel</span></a>
  </div>
  <table class="table tb-type2 nobdb">
    <thead>
      <tr class="thead">
        ...
        <th ><?php echo $lang['admin_predeposit_cash_price']; ?>(<?php echo $lang['currency_zh']; ?>)</th>
          <th>手续费(<?php echo $lang['currency_zh']; ?>)</th>
          <th >实际所得(<?php echo $lang['currency_zh']; ?>)</th>
    ...
    <tbody>
      ...
      <tr class="hover">
        ...
        <td><?php echo $v['pdc_member_name']; ?></td>
          <td>
              <?php echo '开户行:' . $v['pdc_bank_name'] . '<br/>姓名:' . $v['pdc_bank_user'] . '<br/>帐号:' . $v['pdc_bank_no'];?>
          </td>
        <td class="nowrap align-center"><?php echo @date('Y-m-d H:i:s',$v['pdc_add_time']);?></td>
        <td><?php echo $v['pdc_amount']; ?></td>
          <td><?php echo $v['pdc_amount'] * 0.03; ?></td>
          <td><?php echo $v['pdc_amount'] - $v['pdc_amount'] * 0.03; ?></td>
        ...
        <td class="w90 align-center">
          ...
          <a href="javascript:void(0)" onclick="if(confirm('<?php echo $lang['nc_ensure_del']; ?>')){window.location='index.php?act=predeposit&op=pd_cash_del&id=<?php echo $v['pdc_id']; ?>';}else{return false;}"><?php echo $lang['nc_del']; ?></a>
              |
              <a class="btn" href="javascript:if (confirm('<?php echo $lang['admin_predeposit_cash_confirm'];?>')){window.location.href='index.php?act=predeposit&op=pd_cash_pay&id=<?php echo $v['pdc_id']; ?>';}else{}"><span>确认</span></a>
