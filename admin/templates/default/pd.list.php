    ...
    <tbody>
      ...
      <tr>
        ...
        <ul>
            ...
            <li>
                <p style="font-size: 16px;line-height: 20px;font-weight:bold;">预存款总充值金额  ： <span style="color:red"><?php echo $output['count']['pdr']?></span>
                    <?php if(!empty($output['count_g']) && is_array($output['count_g'])){ ?>元 =
                        <?php foreach($output['count_g'] as $k=>$v){?>
                            <?php echo $v['pdr_payment_name']=empty($v['pdr_payment_name'])? "无效或未支付" : $v['pdr_payment_name'];?> : <?php echo '<span style="color:red">'.$v['pdr']."</span>元".$i=(empty($output['count_g'][$k+1]))? '':'+';?>
                        <?php }?>
                    <?php }?>
                </p>
            </li>
