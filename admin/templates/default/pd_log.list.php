          ...
          <td><input type="text" name="mname" class="txt" value='<?php echo $_GET['mname'];?>'></td>
            ...
            <td>
                <select name="lg_type">
                    ...
                    <?php if(!empty($output['lg_type']) && is_array($output['lg_type'])){ ?>
                        <?php foreach($output['lg_type'] as $k => $v){
                          ?>
                            <option value="<?php echo $k;?>" <?php if($_GET['lg_type'] == $k){?>selected<?php }?>><?php echo $v;?></option>
                        <?php } ?>
                    <?php } ?>
                </select>
            </td>
      ...
      <tr>
        <td><ul>
            <li><?php echo $lang['admin_predeposit_log_help1'];?></li>
                <li>
                    <p style="font-size: 16px;line-height: 20px;font-weight:bold;">总计  ： <span style="color:red"><?php echo $output['amount']?></span>
                    </p>
                </li>
          </ul></td>
      </tr>
  ...
  <table class="table tb-type2">
    <thead>
      <tr class="thead">
        ...
        <th>当前余额(<?php echo $lang['currency_zh'];?>)</th>
    ...
    <tbody>
      ...
      <tr class="hover">
        ...
        <td><?php echo ncPriceFormat($v['lg_av_amount']) ? ($v['lg_av_amount'] > 0 ? '+' : null ).ncPriceFormat($v['lg_av_amount']) : null;?></td>
        <td><?php echo ncPriceFormat($v['lg_freeze_amount']) ? ($v['lg_freeze_amount'] > 0 ? '+' : null ).ncPriceFormat($v['lg_freeze_amount']) : null;?></td>
        <td><?php echo ncPriceFormat($v['lg_predeposit']);?></td>
