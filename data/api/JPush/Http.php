    ...
    private static function sendRequest($client, $url, $method, $body=null, $times=1) {
        ...
        if ($errorCode) {
            ...
            if ($times < $retries) {
                ...
                if ($errorCode === 28) {
                    throw new Exception("Response timeout. Your request has probably be received by JPush Server,please check that whether need to be pushed again.");
                } elseif ($errorCode === 56) {
                // resolve error[56 Problem (2) in the Chunked-Encoded data]
                    throw new Exception("Response timeout, maybe cause by old CURL version. Your request has probably be received by JPush Server, please check that whether need to be pushed again.");
                } else {
                    throw new Exception("Connect timeout. Please retry later. Error:" . $errorCode . " " . curl_error($ch));
                }
            }
        } else {
