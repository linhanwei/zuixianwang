...
class statModel extends Model{
    ...
    public function getBillList($condition,$type,$have_page=true){
    	switch ($type){
    		case 'os'://平台
    			return $this->field('sum(os_order_totals) as oot,sum(os_order_return_totals) as oort,sum(os_commis_totals-os_commis_return_totals) as oct,sum(os_store_cost_totals) as osct,sum(os_result_totals) as ort,sum(oc_commis_totals) as oc_commis_totals,sum(oc_commis_return_totals) as oc_commis_return_totals')->table('order_statis')->where($condition)->select();
    			break;
    		case 'ob'://店铺
    			$page = $have_page?15:'';
