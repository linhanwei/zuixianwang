...
class pointsModel {
	/**
	 * 操作会员积分
	 * @param  string $stage 操作阶段 regist(注册),login(登录),comments(评论),order(下单),system(系统),other(其他),pointorder(会员积分礼品兑换),app(同步会员积分兑换)
	 * @param  array $insertarr 该数组可能包含信息 array('pl_memberid'=>'会员编号','pl_membername'=>'会员名称','pl_adminid'=>'管理员编号','pl_adminname'=>'管理员名称','pl_points'=>'会员积分','pl_desc'=>'描述','orderprice'=>'订单金额','order_sn'=>'订单编号','order_id'=>'订单序号','point_ordersn'=>'会员积分兑换订单编号');
	...
	function savePointsLog($stage,$insertarr,$if_repeat = true){
		...
		switch ($stage){
            case 'upgrade':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '升级获得会员积分';
                }
                break;
            case 'distill':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '提取奖励';
                }
                $insertarr['pl_points'] = $insertarr['distill_amount'];
                break;
            case 'transfer':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '消费获得会员积分';
                }
                //返还一半
                $insertarr['pl_points'] = round($insertarr['transfer_amount'] * 0.5,2);
                break;
			case 'regist':
				if (!$insertarr['pl_desc']){
			...
			case 'order':
				...
				$insertarr['pl_points'] = round($insertarr['orderprice']*0.5,2);
                /*
			...
			case 'pointorder':
				if (!$insertarr['pl_desc']){
					$insertarr['pl_desc'] = '兑换礼品信息'.$insertarr['point_ordersn'].'消耗会员积分';
				}
				break;
            case 'redeemable':
                if(!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '返还至帐户余额';
                }
                $insertarr['pl_points'] = -1 * $insertarr['pl_points'];
                break;
            case 'app':
				if (!$insertarr['pl_desc']){
		...
		if($value_array['pl_points'] != 0){
            ...
            if ($result){
                //统计每天数量
                if($stage != 'redeemable'){
                    Model('points_sum')->saveSum($insertarr['pl_memberid'],$insertarr['pl_membername'],$insertarr['pl_points']);
                }
                ...
                $result = $obj_member->editMember(array('member_id'=>$insertarr['pl_memberid']),$upmember_array);
                return $result;
            }else {
