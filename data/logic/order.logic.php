...
class orderLogic {
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '') {
        try {
            ...
            $sellerInfo = Model('seller')->getSellerInfo(array('store_id'=>$order_info['store_id'],'is_admin'=>1));
            //消费返还
            $consume  = Logic('consume')->consume($order_info['buyer_id'],$order_info['buyer_name'],$sellerInfo['member_id'],$sellerInfo['seller_name'],$order_info['order_amount']-$order_info['order_coin'],'商城购物');
            ...
            $amount = Logic('consume')->consume($order_info['buyer_id'],$order_info['buyer_name'],$sellerInfo['member_id'],$sellerInfo['seller_name'],$order_info['order_amount'],'',true);
            if($amount>0){
                return callback(true,'操作成功');
            }
        } catch (Exception $e) {
