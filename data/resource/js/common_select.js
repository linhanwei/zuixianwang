...
/* 地区选择函数 */
function regionAgentInit(divId){
    var area_id = 0;
    getArea(function(){
        if(typeof(nc_a[area_id]) == 'object' && nc_a[area_id].length > 0){//数组存在
            var area_select = $("#" + divId + " > select");//选择要初始化的对象
            areaAgentInit(area_select,area_id);
        }
        $("#" + divId + " > select").change(regionAgentChange); // select的onchange事件
        $("#" + divId + " > input:button[class='edit_region_agent']").click(regionAgentEdit); // 编辑按钮的onclick事件
    });
}
function areaAgentInit(area_select,area_id){//初始化地区
    getArea(function(){
        if(typeof(area_select) == 'object' && nc_a[area_id].length > 0){
            var areas = new Array();
            areas = nc_a[area_id];
            ...
            for (i = 0; i <areas.length; i++){
                area_id = areas[i][0];
                area_name = areas[i][1];
                /*var url = 'index.php?act=member&op=checkAgent&area_id='+area_id;
                $.getJSON(url, {}, function(data){
                    if (data == true)
                    {
                       $(area_select).append("<option value='" + area_id + "'>" + area_name + "</option>");
                    }
                });
                */
                $(area_select).append("<option value='" + area_id + "'>" + area_name + "</option>");
...
if(typeof(regionAgentChange) != 'function'){//检测是否已经被定义过，防止重写
    function regionAgentChange(){
        // 删除后面的select
        $(this).nextAll("select").remove();
        // 计算当前选中到id和拼起来的name
        var selects = $(this).siblings("select").andSelf();
        ...
        $(".area_agent_ids").val(id);
        $(".area_agent_name").val(name);
        $(".area_agnet_names").val(names.join("\t"));

        if (this.value > 0){//下级地区
            var area_id = this.value;
            if(typeof(nc_a[area_id]) == 'object' && nc_a[area_id].length > 0){//数组存在
                $("<select></select>").change(regionAgentChange).insertAfter(this);
                areaAgentInit($(this).next("select"),area_id);//初始化地区
            }
        }
    }
}

function regionAgentEdit()
{
    $(this).siblings("select").show();
    $(this).siblings("span").andSelf().hide();
}
