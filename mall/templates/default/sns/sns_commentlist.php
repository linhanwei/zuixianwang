	...
	$("[nc_type='commentbtn']").die().live('click',function(){
		...
		if($("#commentform_"+data.txtid).valid()){			
			...
			if(cookienum >= <?php echo $output['max_recordnum'];?> && $("#commentseccode"+data.txtid).css('display')=="none"){
				...
				var hash = $("#commentseccode"+data.txtid).find("[name='hash']").val();
				$("#commentseccode"+data.txtid).find("[name='codeimage']").attr('src','index.php?act=seccode&op=makecode&hash='+hash+'&t=' + Math.random());
