...
class loginControl extends BaseHomeControl {
	...
	public function indexOp(){
		...
		if ($result !== false){
			...
			if (process::islock('login')) {
				showDialog($lang['nc_common_op_repeat'],MALL_SITE_URL,'','error',$script);
			}
			...
			if ($error != ''){
			    showDialog($error,MALL_SITE_URL,'error',$script);
			}
			$array	= array();
	...
	public function bind_emailOp() {
	   ...
	   if (!is_numeric($member_id)) {
	       showMessage('验证失败',MALL_SITE_URL,'html','error');
	   }
