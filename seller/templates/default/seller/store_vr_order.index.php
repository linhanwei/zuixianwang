...
<div class="tabmenu">
  ...
  <a href="<?php echo urlShop('store_vr_order', 'exchange');?>" class="ncsc-btn ncsc-btn-orange"><i class="icon-edit"></i>兑换兑换码</a>
</div>
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    ...
    <tr>
      ...
      <td>
      <input type="checkbox" id="skip_off" value="1" <?php echo $_GET['skip_off'] == 1 ? 'checked="checked"' : null;?>  name="skip_off"> <label for="skip_off">不显示已关闭的订单</label></td>
      <?php } ?>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="tl"><dl class="goods-name">
          <dt><a target="_blank" href="<?php echo urlShop('goods','index',array('goods_id'=>$order['goods_id']));?>"><?php echo $order['goods_name']; ?></a></dt>
          <dd>
...
<?php } ?>
      ...
      <td class="bdl bdr"><p><?php echo $order['state_desc']; ?>
        </p>

        <!-- 订单查看 -->
        <p><a href="index.php?act=store_vr_order&op=show_order&order_id=<?php echo $order['order_id'];?>" target="_blank"><?php echo $lang['store_order_view_order'];?></a></p>
        </td>
      <td class="bdl bdr">
      <!-- 取消订单 -->
        <?php if($order['if_cancel']) { ?>
        <p><a href="javascript:void(0)" class="ncsc-btn ncsc-btn-red mt5" nc_type="dialog" uri="index.php?act=store_vr_order&op=change_state&state_type=order_cancel&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_cancel" /><i class="icon-remove-circle"></i><?php echo $lang['store_order_cancel_order'];?></a></p>
        <?php } ?>
    </tr>
    <?php } } else { ?>
    <tr>
