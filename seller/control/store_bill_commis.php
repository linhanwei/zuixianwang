...
class store_bill_commisControl extends BaseSellerControl {
    /**
     * 每次导出多少条记录
     * @var unknown
     */
    ...
    public function indexOp() {
    	...
    	$member_agent = $model_member->getAgentList(54);
        ...
        if($bill_list){
        	foreach($bill_list as $key=>$bill_info){
        		//取得店铺获得的佣金
				$commis_info = $model_bill->getOrderCommisInfo(array('ob_no'=>$bill_info['ob_no'],'dai_type'=>5));
				
				$bill_list[$key]['oc_commis_totals'] = $commis_info['oc_commis_totals'];
				$bill_list[$key]['oc_commis_return_totals'] = $commis_info['oc_commis_return_totals'];
        	}
        }
        ...
        Tpl::showpage('store_bill_commis.index');
    }
	...
	public function show_billOp(){
		...
		$bill_info['oc_commis_totals'] = $commis_info['oc_commis_totals'];
		$bill_info['oc_commis_return_totals'] = $commis_info['oc_commis_return_totals'];
		...
		if ($_GET['type'] =='refund'){
			...
			$refund_list = $model_refund->getRefundReturnList($refund_condition,20,'*');
		    ...
		    if (is_array($refund_list)) {
		    	$commis_list = Model('bill')->getOrderCommissionList(array('ob_no'=>$bill_info['ob_no']),'*',10,'dai_type');
		    	$commis_count = count($commis_list);
		    	$commis_amount = 0;
		        foreach ($refund_list as $refund_key =>$refund_info) {
		        	$commis_amount = $refund_info['refund_amount'];
		        	
		        	for($i = 0;$i<$commis_count;$i++){
				        //当前佣金
		    			$commis_amount = $commis_amount * ($commis_list[$i]['oc_commis_rate']/100);
		    			//下一级的佣金
		    			$next_commis = 0;
		    			if($i+1 < $commis_count){
		    				$next_commis = $commis_amount * ($commis_list[$i+1]['oc_commis_rate']/100);
		    			}
		    			
		        	}
		        	
	    			$refund_list[$refund_key]['commis_amount'] = round($commis_amount - $next_commis,2); 
		        }
		    }
			//取返还佣金
			Tpl::output('refund_list',$refund_list);
			Tpl::output('show_page',$model_refund->showpage());
			$sub_tpl_name = 'store_bill_commis.show.refund_list';
		    ...
		    if (is_array($order_list)) {
		    	...
		    	$commis_count = count($commis_list);
		    	$commis_amount = 0;
		        foreach ($order_list as $order_key =>$order_info) {
		        	$commis_amount = $order_info['order_amount'];
	    			...
	    			$order_list[$order_key]['commis_amount'] = round($commis_amount - $next_commis,2); 
		        }
		    }
		    
		    Tpl::output('order_list',$order_list);
		    Tpl::output('show_page',$model_order->showpage());
		    $sub_tpl_name = 'store_bill_commis.show.order_list';
		    $this->profile_menu('show','order_list');
		}

		Tpl::output('sub_tpl_name',$sub_tpl_name);
		Tpl::output('bill_info',$bill_info);
		Tpl::showpage('store_bill_commis.show');
	}
	...
	public function bill_printOp(){
		...
		Tpl::output('bill_info',$bill_info);
		Tpl::showpage('store_bill_commis.print','null_layout');
	}
	...
	public function export_orderOp(){
		...
		if (!is_numeric($_GET['curpage'])){
    		...
    		if ($count > self::EXPORT_SIZE ){
    		    ...
    		    Tpl::output('murl','index.php?act=store_bill_commis&op=show_bill&ob_no='.$_GET['ob_no']);
		...
		//然后计算订单的佣金
	    if (is_array($data)) {
	    	$commis_list = $model_bill->getOrderCommissionList(array('ob_no'=>$bill_info['ob_no']),'*',10,'dai_type');
	    	$commis_count = count($commis_list);
	    	$commis_amount = 0;
	        foreach ($data as $key =>$val) {
	        	$commis_amount = $val['order_amount'];
    			...
    			$data[$key]['commis_amount'] = round($commis_amount - $next_commis,2); 
	        }
	    }
		...
		foreach ($data as $v) {
		    ...
		    $commis_totals += $export_data[$k+1][] = $v['commis_amount'];
	...
	public function export_refund_orderOp(){
	    ...
	    if (!is_numeric($_GET['curpage'])){
	        ...
	        if ($count > self::EXPORT_SIZE ){	//显示下载链接
	            ...
	            Tpl::output('list',$array);
	            Tpl::output('murl','index.php?act=store_bill_commis&op=show_bill&query_type=refund&ob_no='.$_GET['ob_no']);
	            Tpl::showpage('store_export.excel');
	            exit();
	        }else{
	            //如果数量小，直接下载
	            $data = $model_refund->getRefundReturnList($condition,'','*',self::EXPORT_SIZE);
	        }
	    }else{
	        //下载
	        $limit1 = ($_GET['curpage']-1) * self::EXPORT_SIZE;
	        $limit2 = self::EXPORT_SIZE;
	        $data = $model_refund->getRefundReturnList(condition,'','*',"{$limit1},{$limit2}");
	    }
	    ...
	    if (is_array($data) && $data) {
	    	...
	    	$commis_amount = 0;
	        foreach ($data as $key =>$val) {
	        	$commis_amount = $val['refund_amount'];
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'list':
				$menu_array = array(
				1=>array('menu_key'=>'list','menu_name'=>'实物订单佣金结算', 'menu_url'=>'index.php?act=bill&op=list'),
				);
				break;
			case 'show':
				$menu_array = array(
				array('menu_key'=>'order_list','menu_name'=>'订单列表', 'menu_url'=>'index.php?act=store_bill_commis&op=show_bill&ob_no='.$_GET['ob_no']),
				array('menu_key'=>'refund_list','menu_name'=>'退款订单',	'menu_url'=>'index.php?act=store_bill_commis&op=show_bill&type=refund&ob_no='.$_GET['ob_no'])
				);
				break;
		}
