...
final class Email{
	...
	private function html($subject, $message){
        $tmp = '';
		$message = preg_replace("/href\=\"(?!http\:\/\/)(.+?)\"/i", 'href="'.YP_SITE_URL.'\\1"', $message);
		$tmp .= "<html><head>";
	...
	private function header($from=''){
		...
		$header .= "X-Mailer: {$this->email_delimiter}";
		...
		$header .= 'Message-ID: <'.gmdate('YmdHs').'.'.substr(md5(microtime()), 0, 6).rand(100000, 999999).'@'.$_SERVER['HTTP_HOST'].">{$this->email_delimiter}";
		return $header;
	}
