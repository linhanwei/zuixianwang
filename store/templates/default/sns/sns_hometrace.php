    ...
    <div class="release-content"><span class="arrow"></span>
      <form id="weiboform" method="post" action="index.php?act=member_snsindex&op=addtrace">
        ...
        <div id="weiboseccode" class="weiboseccode">
          ...
          <a href="javascript:void(0)" class="ml5 fl"><img src="" title="<?php echo $lang['wrong_checkcode_change'];?>" name="codeimage" border="0" id="codeimage" onclick="this.src='index.php?act=seccode&op=makecode&hash=<?php echo $output['hash'];?>&t=' + Math.random()" /></a>
	...
	$("#weibobtn").bind('click',function(){			
		if($("#weiboform").valid()){
			...
			if(cookienum >= max_recordnum && $("#weiboseccode").css('display') == 'none'){
				//显示验证码
				$("#weiboseccode").show();
				$("#weiboseccode").find("#codeimage").attr('src','index.php?act=seccode&op=makecode&hash=<?php echo $output['hash'];?>&t=' + Math.random());
