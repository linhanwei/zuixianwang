  ...
  <div id="pictureFolder" class="album">
    <div class="intro mb20">
      <div class="covers"><span class="thumb size60"><i></i><a href="index.php?act=sns_album&op=album_pic_list&id=<?php echo $output['class_info']['ac_id']?>">
        ...
        <img id="aclass_cover" src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$output['master_id'].DS.$output['class_info']['ac_cover'];?>"  onerror="this.src='<?php echo MALL_TEMPLATES_URL.'/images/member/default_image.png';?>'" onload="javascript:DrawImage(this,60,60);">
        <?php }else{?>
        <img id="aclass_cover" src="<?php echo MALL_TEMPLATES_URL.'/images/member/default_image.png';?>" onload="javascript:DrawImage(this,60,60);">
        <?php }?>
        </a></span></div>
      <dl>
...
<script type="text/javascript">
  $(function() {
    ...
    var galleries = $('.ad-gallery').adGallery({loader_image:'<?php echo MALL_TEMPLATES_URL;?>/images/loading.gif', start_at_index:<?php echo intval($_GET['start_index'])?>, slideshow:{enable: false,start_label: '<?php echo $lang['album_pinfo_autoplay'];?>', stop_label: '<?php echo $lang['album_pinfo_suspend'];?>'}});
...
function ajax_change_imgmessage(url){
	...
	$.getJSON("<?php echo STORE_SITE_URL; ?>/index.php?act=sns_album&op=ajax_change_imgmessage&mid=<?php echo $output['master_id'];?>", {'url':url}, function(data){
