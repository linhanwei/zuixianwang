...
.dialog-share .share-widget .s-app i { background: url("<?php echo MALL_TEMPLATES_URL;?>/images/member/shareicon/shareicons.png") no-repeat 0 0; display: block; width: 24px; height: 24px; cursor: pointer;}
...
.dialog-share .share-widget .s-app .tip .arrow { background: url("<?php echo MALL_TEMPLATES_URL;?>/images/member/shareicon/shareicons.png") no-repeat -36px 0; display: block; width: 11px; height: 6px; position: absolute; z-index: 2; top: -6px; left: 20px;}
...
.share-privacy .privacytab .arrow { background: url("<?php echo MALL_TEMPLATES_URL;?>/images/member/shareicon/shareicons.png") no-repeat -24px 0; display: block; width: 11px; height: 6px; margin-right: -6px; position: absolute; z-index: 2; top: -6px; right: 50%;}
...
.share-privacy .privacytab ul li .selected { background: url("<?php echo MALL_TEMPLATES_URL;?>/images/member/shareicon/shareicons.png") no-repeat -32px -20px;}
...
.dialog-share .share-content .avatar i { background: url("<?php echo MALL_TEMPLATES_URL;?>/images/member/shareicon/shareicons.png") no-repeat -30px -50px; width: 10px; height: 10px; position: absolute; z-index: 1; top: 10px; right: -15px;}
...
<div class="dialog-share">
  <form method="post" action="index.php?act=member_snsindex&op=sharestore" id="sharestore_form">
    ...
    <div id="ss_seccode" class="seccode"><?php echo $lang['nc_checkcode'].$lang['nc_colon']; ?>
      ...
      <img src="" title="<?php echo $lang['wrong_checkcode_change'];?>" name="codeimage" onclick="this.src='index.php?act=seccode&op=makecode&hash=<?php echo $output['hash'];?>&t=' + Math.random()"/><span><?php echo $lang['wrong_seccode'];?></span>
      <input type="hidden" name="hash" value="<?php echo $output['hash'];?>"/>
    </div>
	...
	$("#weibobtn_store").click(function(){
		if($("#sharestore_form").valid()){
			...
			if(cookienum >= max_recordnum && $("#ss_seccode").css('display') == 'none'){
				//显示验证码
				$("#ss_seccode").show();
				var hash = $("#ss_seccode").find("[name='hash']").val();
				$("#ss_seccode").find("[name='codeimage']").attr('src','index.php?act=seccode&op=makecode&hash='+hash+'&t=' + Math.random());
