...
$(function(){
    ...
    $('a[nctype="del"]').unbind().click(
        function() {
            ...
            obj.find('img:first').attr('src', MALL_TEMPLATES_URL + "/images/loading.gif");
            ...
            $.getScript(MALL_RESOURCE_SITE_URL + "/js/store_slide.js");
        });
});
...
function ajaxFileUpload(url, id, file_id)
{
        $('div[nctype="'+id+'"]').find('i').remove().end().find('img').remove()
            .end().prepend('<img nctype="'+id+'" scr="'+MALL_TEMPLATES_URL+'/images/loading.gif">');
	$('img[nctype="'+id+'"]').attr('src',MALL_TEMPLATES_URL+"/images/loading.gif");
	...
	(
		{
			...
			success: function (data, status)
			{
				...
				$.getScript(MALL_RESOURCE_SITE_URL+"/js/jquery.flexslider-min.js");
				$.getScript(MALL_RESOURCE_SITE_URL+"/js/store_slide.js");
			},
