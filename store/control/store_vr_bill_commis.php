...
class store_vr_bill_commisControl extends BaseSellerControl {
    /**
     * 每次导出多少条记录
     * @var unknown
     */
    ...
    public function indexOp() {
        ...
        Tpl::showpage('store_vr_bill_commis.index');
    }
	...
	public function show_billOp(){
		...
		Tpl::output('sub_tpl_name','store_vr_bill_commis.show.code_list');
		Tpl::output('bill_info',$bill_info);
		$this->profile_menu('show',$_GET['type']);
		Tpl::showpage('store_vr_bill_commis.show');
	}
	...
	public function bill_printOp(){
		...
		Tpl::output('bill_info',$bill_info);
		Tpl::showpage('store_vr_bill_commis.print','null_layout');
	}
	...
	public function export_orderOp(){
		...
		if (!is_numeric($_GET['curpage'])){
    		...
    		if ($count > self::EXPORT_SIZE ){
    		    ...
    		    Tpl::output('murl','index.php?act=store_vr_bill_commis&op=show_bill&ob_no='.$_GET['ob_no']);
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'list':
				$menu_array = array(
				1=>array('menu_key'=>'list','menu_name'=>'虚拟订单佣金结算', 'menu_url'=>'index.php?act=bill&op=list'),
				);
				break;
			case 'show':
				$menu_array = array(
				array('menu_key'=>'','menu_name'=>'已使用', 'menu_url'=>'index.php?act=store_vr_bill_commis&op=show_bill&ob_no='.$_GET['ob_no']),
				array('menu_key'=>'timeout','menu_name'=>'已过期',	'menu_url'=>'index.php?act=store_vr_bill_commis&op=show_bill&type=timeout&ob_no='.$_GET['ob_no'])
				);
				break;
		}
