...
class member_snsindexControl extends BaseMemberControl {
	...
	public function indexOp(){
		...
		//信息输出
		Tpl::output('hash',substr(md5(STORE_SITE_URL.$_GET['act'].$_GET['op']),0,8));
	...
	public function addtraceOp(){
		...
		if ($result){
			...
			if (cookie('weibonum') != null && intval(cookie('weibonum')) >0){
				setMyCookie('weibonum',intval(cookie('weibonum'))+1,2*3600);//保存2小时
			}else{
				setMyCookie('weibonum',1,2*3600);//保存2小时
			}
	...
	public function tracelistOp(){
		...
		if (!empty($tracelist)){
			foreach ($tracelist as $k=>$v){
				if ($v['trace_title']){
					$v['trace_title'] = str_replace("%siteurl%", STORE_SITE_URL.DS, $v['trace_title']);
				...
				if(!empty($v['trace_content'])){
					//替换内容中的siteurl
					$v['trace_content'] = str_replace("%siteurl%", STORE_SITE_URL.DS, $v['trace_content']);
				}
				$tracelist[$k] = $v;
			}
		}
	...
	public function addcommentOp(){
		...
		if ($result){
			...
			if (cookie('commentnum') != null && intval(cookie('commentnum')) >0){
				setMyCookie('commentnum',intval(cookie('commentnum'))+1,2*3600);//保存2小时
			}else{
				setMyCookie('commentnum',1,2*3600);//保存2小时
			}
		...
		if ($result){
			...
			if ($_GET['irefresh']){
				...
				if (cookie('forwardnum') != null && intval(cookie('forwardnum')) >0){
					setMyCookie('forwardnum',intval(cookie('forwardnum'))+1,2*3600);//保存2小时
				}else{
					setMyCookie('forwardnum',1,2*3600);//保存2小时
				}
				if ($_GET['type']=='home'){
