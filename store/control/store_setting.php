...
class store_settingControl extends BaseSellerControl {
    ...
    public function store_settingOp() {
        ...
        if (chksubmit()) {
            ...
            if (!empty($store_domain) && $store_domain != $store_info['store_domain']) {
                ...
                if ($subdomain_times > $store_domain_times) {//可继续修改
                    ...
                    if (!empty($store_info['store_domain']))
                        $param['store_domain_times'] = $store_domain_times + 1; //第一次保存不计数
            ...
            if ($_FILES['banner_1']['name'] != '') {
                $param['banner_1'] = $this->upload_image('banner_1',$store_info['banner_1'],$upload);
            }
            if ($_FILES['banner_2']['name'] != '') {
                $param['banner_2'] = $this->upload_image('banner_2',$store_info['banner_2'],$upload);
            }
            if ($_FILES['banner_3']['name'] != '') {
                $param['banner_3'] = $this->upload_image('banner_3',$store_info['banner_3'],$upload);
            }
            if ($_FILES['banner_4']['name'] != '') {
                $param['banner_4'] = $this->upload_image('banner_4',$store_info['banner_4'],$upload);
            }
            if ($_FILES['banner_5']['name'] != '') {
                $param['banner_5'] = $this->upload_image('banner_5',$store_info['banner_5'],$upload);
            }
            
            //店铺详情
            $param['store_content'] = $_POST['store_content'];
            
            //店铺地址
            $param['province_id'] = $_POST['prov'];
            $param['city_id'] = $_POST['city'];
            $param['region_id'] = $_POST['region'];
            if($_POST['area_info']){
                $param['area_info'] = $_POST['area_info'];
            }
            $param['store_address'] = $_POST['store_address'];
            
            /**
             * 更新入库
             */
            $param['store_label'] = empty($_POST['store_label']) ? $store_info['store_label'] : $_POST['store_label'];
            $param['store_banner'] = empty($_POST['store_banner']) ? $store_info['store_banner'] : $_POST['store_banner'];
            $param['store_avatar'] = empty($_POST['store_avatar']) ? $store_info['store_avatar'] : $_POST['store_avatar'];
            $param['store_qq'] = $_POST['store_qq'];
            $param['store_ww'] = $_POST['store_ww'];
            $param['store_phone'] = $_POST['store_phone'];
            $param['store_zy'] = $_POST['store_zy'];
            $param['store_keywords'] = $_POST['seo_keywords'];
            $param['store_description'] = $_POST['seo_description'];
            $param['store_phone'] = $_POST['store_phone'];
            $param['store_description'] = $_POST['seo_description'];
            $param['store_description'] = $_POST['seo_description'];
    ...
    /**
     * 上传图片
     * @param type $file 表单名字
     * @param type $y_img 原图的信息
     * @param type $upload 上传类
     * @return type
     */
    private function upload_image($file,$y_img,$upload) {
        $pic_name = '';
    ...
    public function themeOp() {
         ...
         * 当前店铺预览图片
         */
        $curr_image = MALL_TEMPLATES_URL . '/store/style/' . $curr_store_theme . '/images/preview.jpg';
        $curr_theme = array(
        ...
        foreach ($style_data as $key => $val) {
            if (in_array($key, $themes)) {
                $theme_list[$key] = array(
                    ...
                    'image' => MALL_TEMPLATES_URL . '/store/style/' . $key . '/images/preview.jpg'
                );
            }
        }
        /**
