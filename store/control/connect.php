...
class connectControl extends BaseHomeControl{
	...
	public function registerOp(){
		...
		if (chksubmit()){
			...
			showMessage(Language::get('nc_common_save_succ'),STORE_SITE_URL);
		}else {
			//检查登录状态
			...
			if($result) {
				...
				showMessage(Language::get('login_usersave_regist_fail'),STORE_SITE_URL.'/index.php?act=login&op=register','html','error');//"会员注册失败"
			}
		}
	}
	...
	public function autologin(){
		...
		if (is_array($member_info) && count($member_info)>0){
            ...
            if($this->checkWapQQlogin()){
                ...
                showMessage($success_message,STORE_SITE_URL);
            }
		}
	}
	/**
	...
	public function changeqqOp(){
		...
		unset($_SESSION['openid']);
		@header('Location:'.STORE_SITE_URL.'/api.php?act=toqq');
		exit;
	}
