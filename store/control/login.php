...
class loginControl extends BaseHomeControl {
	...
	public function indexOp(){
		...
		if ($_GET['inajax'] == 1 && C('captcha_status_login') == '1'){
		    $script = "document.getElementById('codeimage').src='".APP_SITE_URL."/index.php?act=seccode&op=makecode&hash=".getUrlhash()."&t=' + Math.random();";
		}
		...
		if ($result !== false){
			...
			if (process::islock('login')) {
				showDialog($lang['nc_common_op_repeat'],STORE_SITE_URL,'','error',$script);
			}
			...
			if ($error != ''){
			    showDialog($error,STORE_SITE_URL,'error',$script);
			}
			$array	= array();
	...
	public function logoutOp(){
		...
		$lang	= Language::getLangContent();
		// 清理消息COOKIE
		setMyCookie('msgnewnum'.$_SESSION['member_id'],'',-3600);
		session_unset();
		session_destroy();
	...
	public function bind_emailOp() {
	   ...
	   if (!is_numeric($member_id)) {
	       showMessage('验证失败',STORE_SITE_URL,'html','error');
	   }
	   ...
	   if (!$update) {
	       showMessage('系统发生错误，如有疑问请与管理员联系',STORE_SITE_URL,'html','error');
	   }
