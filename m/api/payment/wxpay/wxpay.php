...
class wxpay{
    ...
    private $appid = '';
    private $redirect_uri = '/api/payment/wxpay/redirect_uri.php';
    private $notify_url = '/api/payment/wxpay/notify_url.php';
    ...
    public function __construct() {
        ...
        $this->redirect_uri = MOBILE_SITE_URL . $this->redirect_uri;
        $this->notify_url = MOBILE_SITE_URL . $this->notify_url;
     ...
     * 返回app处理
     */
    public function prePay($orderparam){
        $order_sn = $orderparam['order_type'].$orderparam['order_sn'];
        if(!$order_sn){
            output_error('取支付授权失败1');
            die;
        }else{
            $this->dowxpay('', $order_sn,'APP');
        }
    }


    private function dowxpay($openid, $order_sn,$trade_type = 'JSAPI'){
