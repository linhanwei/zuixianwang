...
class alipay {
	...
	public function submit($param){
		...
		$this->out_trade_no		= $param['order_type'] . $param['order_sn'];
    ...
    public function getReturnInfo($payment_config) {
        ...
        if($verify) {
            return array(
                //商户订单号
                'out_trade_no' => substr($_GET['out_trade_no'],1),
                'order_type' => substr($_GET['out_trade_no'],0,1),
                //支付宝交易号
    ...
    public function getNotifyInfo($payment_config) {
        ...
        if($verify) {
            ...
            if($trade_status == 'TRADE_FINISHED' || $trade_status == 'TRADE_SUCCESS') {
                if($payment_config['alipay_partner'] != $_POST['seller_id']){
                    return false;
                }
                ...
                return array(
                    ...
                    'order_type' => substr($out_trade_no,0,1),
                    //支付宝交易号
                    'trade_no' => $trade_no,
